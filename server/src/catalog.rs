...
pub async fn handle(request: &[u8], pricing: &PricingTable, margin: f64) -> Vec<u8> {
    ...
    if let Some(models) = cached_fresh() {
        return serde_json::to_vec(&json!({ "id": id, "models": models, "testnet": crate::pay::is_testnet_server(), "faucetUrl": crate::pay::faucet_url() })).unwrap_or_default();
    }
    ...
    serde_json::to_vec(&json!({ "id": id, "models": models, "testnet": crate::pay::is_testnet_server(), "faucetUrl": crate::pay::faucet_url() })).unwrap_or_default()
}
