...
pub async fn handle(request: &[u8], pricing: &PricingTable, margin: f64, identities: &[String]) -> Vec<u8> {
    ...
    if let Some(models) = cached_fresh() {
        return serde_json::to_vec(&json!({ "id": id, "models": models, "testnet": crate::pay::is_testnet_server(), "faucetUrl": crate::pay::faucet_url(), "siteUrl": crate::pay::site_url(), "card": crate::pay::card_info().await, "rails": crate::pay::rails_info(), "serverVersion": crate::VERSION, "identities": identities })).unwrap_or_default();
    }
    ...
    serde_json::to_vec(&json!({ "id": id, "models": models, "testnet": crate::pay::is_testnet_server(), "faucetUrl": crate::pay::faucet_url(), "siteUrl": crate::pay::site_url(), "card": crate::pay::card_info().await, "rails": crate::pay::rails_info(), "serverVersion": crate::VERSION, "identities": identities })).unwrap_or_default()
}
