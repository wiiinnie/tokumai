...
pub async fn handle(request: &[u8], pricing: &PricingTable, margin: f64, identities: &[String]) -> Vec<u8> {
    ...
    if let Some(models) = cached_fresh() {
        return serde_json::to_vec(&json!({ "id": id, "models": models, "testnet": crate::pay::is_testnet_server(), "faucetUrl": crate::pay::faucet_url(), "siteUrl": crate::pay::site_url(), "card": crate::pay::card_info().await, "rails": crate::pay::rails_info(), "coins": crate::pay::coins_info(), "trickleMinutes": crate::pay::trickle_minutes(), "redeemLabel": crate::pay::redeem_label().0, "redeemHint": crate::pay::redeem_label().1, "serverVersion": crate::VERSION, "tender64": true, "purchaseTiers": crate::pay::purchase_tiers(), "identities": identities, "iapProducts": crate::iap::product_ids(), "iapPlans": crate::iap::plan_ids(), "plans": crate::pay::plans_info() })).unwrap_or_default();
    }
