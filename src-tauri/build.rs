fn main() {
    // iOS: the StoreKit 2 bridge (gen/apple/.../StoreKitShim.swift) defines the C symbols
    ...
    // the linker is told they may stay undefined. The staticlib Xcode consumes is
    // unaffected and resolves them against the Swift objects.
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("ios") {
        for sym in [
            "tokumai_iap_products",
            "tokumai_iap_purchase",
            "tokumai_iap_unfinished",
            "tokumai_iap_finish",
            "tokumai_iap_entitlements",
        ] {
