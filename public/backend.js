...
const httpBackend = {
  ...
  // Support needs the mixnet and a server; the dev bridge has neither. It answers enough
  // for the screens to render and be clicked through.
  supportSend: () => Promise.resolve({ ticket: "TKM-DEV1" }),
  supportFetch: () => Promise.resolve({ threads: [] }),
  supportList: () => Promise.resolve({ threads: [] }),
  supportSeen: () => Promise.resolve(),
  supportDiag: () => Promise.resolve("app      dev bridge\nos       browser\n"),
  setFakeOldVersion: () => Promise.reject(new Error("only in a development build of the app")),
  ...
  // Plans outside the App Store (Stripe). The dev bridge has no purchase client.
  planLadder: () => Promise.resolve(null),
  planCheckout: () => Promise.reject(new Error("plans are bought in the app, not in the dev bridge")),
  planPoll: () => Promise.resolve({ status: "none" }),
  planForget: () => Promise.resolve({ ok: true }),
  iapPlans: () => Promise.reject(new Error("App Store plans exist only in the iPhone app")),
  iapSubscribe: () => Promise.reject(new Error("App Store plans exist only in the iPhone app")),
  iapSyncPlan: () => Promise.resolve({ checked: 0, account: null }),
...
const tauriBackend = (invoke) => ({
  ...
  supportSend: (category, subject, body, diag, image) => invoke("support_send", { category, subject, body, diag: diag || null, image: image || null }),
  supportFetch: (replyTo, replyText) => invoke("support_fetch", { replyTo: replyTo || null, replyText: replyText || null }),
  supportList: () => invoke("support_list"),
  supportSeen: (id) => invoke("support_seen", { id }),
  supportDiag: (lastError) => invoke("support_diag", { lastError: lastError || null }),
  setFakeOldVersion: (on) => invoke("set_fake_old_version", { on: !!on }),
  ...
  planCheckout: (tier, yearly) => invoke("plan_checkout", { tier, yearly: !!yearly }),
  planPoll: () => invoke("plan_poll"),
  planForget: () => invoke("plan_forget"),
  iapPlans: () => invoke("iap_plans"),
  iapSubscribe: (productId) => invoke("iap_subscribe", { productId }),
  iapSyncPlan: () => invoke("iap_sync_plan"),
...
export const Backend = {
  ...
  planLadder: () => pick("planLadder"),
  planCheckout: (tier, yearly) => pick("planCheckout", tier, yearly),
  planPoll: () => pick("planPoll"),
  planForget: () => pick("planForget"),
  iapPlans: () => pick("iapPlans"),
  iapSubscribe: (productId) => pick("iapSubscribe", productId),
  iapSyncPlan: () => pick("iapSyncPlan"),
  ...
  supportSend: (category, subject, body, diag, image) => pick("supportSend", category, subject, body, diag, image),
  supportFetch: (replyTo, replyText) => pick("supportFetch", replyTo, replyText),
  supportList: () => pick("supportList"),
  supportSeen: (id) => pick("supportSeen", id),
  supportDiag: (lastError) => pick("supportDiag", lastError),
  setFakeOldVersion: (on) => pick("setFakeOldVersion", on),
