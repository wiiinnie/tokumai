...
| `dynamic-urllib-use-detected` | `scripts/asc.py` | a local developer tool that asks App Store Connect about our own builds. The URL is pinned to the API prefix and refuses a path carrying a scheme; nothing from a network response ever reaches it — annotated `nosemgrep` in place |
